...
public class Secp256k1Blake160ScriptSigner implements ScriptSigner {
  ...
  public boolean signTx(Transaction transaction, ScriptGroup scriptGroup, Context context) {
    ...
    if (isMatched(ecKeyPair, script.args) == false) {
      return false;
    }
  ...
  // Check if the script with `scriptArgs` is generated by and can be unlocked by `keyPair`
  private boolean isMatched(ECKeyPair keyPair, String scriptArgs) {
    if (scriptArgs == null || keyPair == null) {
      return false;
    }

    String publicKeyHash = Numeric.toHexString(Hash.blake2b(keyPair.getPublicKey().toByteArray()));

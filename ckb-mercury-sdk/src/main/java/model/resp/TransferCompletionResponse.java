...
public class TransferCompletionResponse {
    ...
    public List<MercuryScriptGroup> getScriptGroup() {
        ...
        if(this.sigsEntry.size() == 1) {
            MercuryScriptGroup sg = new MercuryScriptGroup(this.sigsEntry.get(0).pubKey,regionToList(this.sigsEntry.get(0).index, this.txView.inputs.size()));
            scriptGroups.add(sg);
            return scriptGroups;
        }
        ...
        for (int i = 0; i < this.sigsEntry.size(); i++) {
            if(i == this.sigsEntry.size() - 1) {
                ...
                MercuryScriptGroup sg = new MercuryScriptGroup(this.sigsEntry.get(i).pubKey,regionToList(start, end));
                scriptGroups.add(sg);
                continue;
            }
